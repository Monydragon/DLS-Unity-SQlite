	...
	public bool draw()
    {
		...
		switch(layout)
        {
                        ...
                        else
                        {
							...
							for(int fi=0;fi<files.Length;fi++)
                            {
								...
								if(files[fi].button())
                                {
                                    ...
                                    fileNameWithExtension = outputFile.Name;
                                    fileName = Path.GetFileNameWithoutExtension(outputFile.FullName);
                                    fileDirectory = outputFile.DirectoryName;
                                    selectedFile = fi;
								}
			...
			default:
                ...
                if ((selectStyle == null) ? GUILayout.Button("Select") : GUILayout.Button("Select", selectStyle))
                {
                    fileDirectoryName = Path.GetDirectoryName(currentDirectory.Name);
                    return true;
                }
